import { FC, useEffect, useState, KeyboardEvent, useRef, ChangeEvent } from 'react';
...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onDeleteTag = (tag: string) => {
    ...
    return setValue('tags', newTags, { shouldValidate: true });
  };
  ...
  const onSubmit = async( form: FormData ) => {
    if ( form.images.length < 2 ) return console.log('Mínimo 2 imagenes');
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
  ...
  if ( slug === 'new' ) {
    // debe de retornar un objecto con la estrucutura del producto
    const tempProduct = JSON.parse( JSON.stringify( new Product() ) );
    // se usa el _id para saber si es un method PUT o un POST
