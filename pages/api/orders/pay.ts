...
const payOrder = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
  try {
    ...
    if ( !paypalBearerToken ) {
      res.status(200).json({ message: 'No se pudo confirmar el token de paypal ' });
    }
    ...
    if ( data.status !== 'COMPLETED' ) {
      return res.status(401).json({ message: 'Orden no reconozida' });
    }
    ...
    //TODO: enviar correo al cliente y al administrador, y si existen ventanas abiertas, notificar con sockets
